...
try {
    ...
    if ( $HORAGPS == 0 )
      // usa hora da IHM
      $sql = "SELECT *, $SQL_COUNTNPONTO FROM (SELECT s.NPONTO as NPT, s.TS as DATAH, s.HORA, 0 as MSEC, s.UTR, s.ESTADO, s.RECON, p.NPONTO, p.ID, p.DESCRICAO, p.FLAGS, p.ESTON, p.ESTOFF, p.UNIDADE, p.TIPO, p.SUPCMD, p.ANOTACAO, p.ESTACAO, p.ESTALM, p.PRIOR ".
             " FROM SOE s ".
             " JOIN DUMPDB p ON p.NPONTO = s.NPONTO ".
             " WHERE $FILTRO_RECONHECIDOS ".
             " AND p.ID !='' $filtclause ".
             $FILTRODATAHORA.
             " ORDER BY DATAH $ORDER2 ) ". 
             $SQL_GROUPBY.
      ...
      $sql = "SELECT *, $SQL_COUNTNPONTO FROM (SELECT s.NPONTO as NPT, s.DATA as DATAH, s.HORA, s.MSEC, s.UTR, s.ESTADO, s.RECON, p.NPONTO, p.ID, p.DESCRICAO, p.FLAGS, p.ESTON, p.ESTOFF, p.UNIDADE, p.TIPO, p.SUPCMD, p.ANOTACAO, p.ESTACAO, p.ESTALM, p.PRIOR ".
             ...
             " AND p.ID !='' $filtclause ".
             $FILTRODATAHORA.
             " ORDER BY DATAH $ORDER2, s.MSEC $ORDER2 ) ".
             $SQL_GROUPBY.
             " ORDER BY DATAH $ORDER, MSEC $ORDER ".
             " LIMIT $NUMMAX_ALARMES+1"; 
